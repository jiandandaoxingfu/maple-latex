...
function tex_format() {
  ...
  let header = lc.match(/.*?\\begin{document}/)[0].replaceAll('AAAAAAAAA', '\n');
  let doc = lc.match(/\\begin{document}(.*?)\\end{document}/)[1].replace(/\\begin{thebibliography}.*?\\end{thebibliography}/, '').replaceAll('AAAAAAAAA', '\n');
  let bib = lc.match(/\\begin{thebibliography}.*?\\end{document}/)[0].replaceAll('AAAAAAAAA', '\n');

  // a. A --> a. \n A
  doc = doc.replace(/([a-z]|\$|})\. {0,}([A-Z])/g, "$1.\n$2");
  // a \begin{} --> a \n \begin{}
  doc = doc.replace(/(.+?)(\\begin\{)/g, '$1\n$2');
  // \begin{}{}[] a --> \begin{}{}[] \n a
  doc = doc.replace(/(\\begin{.*?}\\label{.*?}) *([^\n])/g, '$1\n$2');
  doc = doc.replace(/(\\begin{.*?}{\w+}) *([^\n])/g, '$1\n$2');
  doc = doc.replace(/(\\begin{.*?}\[[^\\]*\]) *([^\n])/g, '$1\n$2');
  doc = doc.replace(/(\\begin{.*?}) *([^\n{\[\\])/g, '$1\n$2');
  // a \end --> a \n \end
  doc = doc.replace(/([^\s]+?)[ \t]*(\\end.*?\})/g, '$1\n$2');
  // \end a --> \end \n a
  doc = doc.replace(/(\\end.*?\})[ \t]*([^\s]+)/g, '$1\n$2');
  // ... \n a --> ... a
  // doc = doc.replace(/[^}]\s+\n+\s+([a-z])/g, '$1');
  // a  a
  doc = doc.replace(/([a-z])\s\s+([a-z])/g, '$1 $2');

  let block_tree = create_block_tree(doc, ['', ''], 0, 3);
  console.log( block_tree );
  doc = recover_from_block_tree(block_tree);
  $$('input').value = ( header + doc + bib.replaceAll('\\bibitem', '\n\\bibitem') ).replace(/(\n *)(\n *)+(\n *)/g, '$1$2');
}
...
function create_block_tree(doc, envir, level, max_level) {
  if( doc.indexOf('\\begin') < 0 ) {
    if( level < 3 ) {
      level = envir[0].match(/(equation|align|tikz|gather|itemize|enumerate|figure|eqnarray)/) ? 3: 0;
    } else {
      level = 3 * (level -1);
    }
    return {
  ...
  return {
    ...
    envir: envir,
    text: doc,
    child: get_blocks(doc).map( doc => create_block_tree(doc, envir, level + 1, max_level) ),
    format_text: ''
  }
}
